# -*- coding: utf-8 -*-
"""
Created on Sun Sep  6 11:41:40 2020
...
# to do event handling-----> when someone presses the button the text must be changed 

# Dash core Components----> to add dropdown etc
...
# Global variables
app = dash.Dash()
#global variables ------>can be accessed everywhere   ,even in main but it cannot be changed
# above variable is referencing dash application
...
def load_data():
  ...
  global df
  df = pd.read_csv(dataset_name)  # df is a local variable it can be used in create_ui or main to use in main it shud be made global
...
# Layout of your page
def create_app8_ui():
  # Creation of the UI of  Webpage 
  # heading and buttonm
    # way to add html components
  main_layout = html.Div([
  html.Div([
  html.H1('Terrorism Analysis with Insights', id='Main_title' ,style={'textAlign':'center' ,'color':'white','font-size':'45px','margin-left':'5%','font-family':'cambria','display':'inline-block'}),
   ],
  style={'margin':'5px -10px 8px','background-color':'indigo'}),
  dcc.Tabs(id="Tabs", value="Map",children=[
      dcc.Tab(label="Map tool" ,id="Map tool",value="Map", children=[
          dcc.Tabs(id = "subtabs", value = "WorldMap",children = [
              ...
              ],style={'font-size':'23px','border':'2px solid indigo','font-family':'cambria'}),
  
    html.Div([        html.Div([html.H3('Apply Filters:',style={'color':'white','font-family':'cambria','padding':'10px','margin-bottom':'0px','margin-top':'2px'})]),
            dcc.Dropdown(
              id='month', 
                options=month_list,
                placeholder='Select Month',
                multi = True,optionHeight=40,
                style={'font-size':'25px','cursor':'pointer','padding':'6px','width':'1000px','height':'55px','background-color':'transparent','borderRadius':'10px'}                  ),
          dcc.Dropdown(
                id='date', 
                placeholder='Select Day',
                multi = True,
                style={'font-size':'25px','cursor':'pointer','width':'1000px','height':'55px','borderRadius':'10px','padding':'6px',
          ...
          dcc.Dropdown(
                ...
                multi = True,optionHeight=40,
                 style={'padding':'6px','font-size':'25px','cursor':'pointer','width':'1000px','height':'55px','background-color':'transparent','borderRadius':'10px'}
          ...
          dcc.Dropdown(
                ...
                multi = True,optionHeight=40,
                style={'font-size':'25px','cursor':'pointer','width':'1000px','height':'55px','background-color':'transparent','borderRadius':'10px','padding':'6px'}
          ...
          dcc.Dropdown(
                ...
                multi = True,optionHeight=40, style={'font-size':'25px','cursor':'pointer','width':'1000px','height':'55px','background-color':'transparent','borderRadius':'10px','padding':'6px'}
                  ),
          dcc.Dropdown(
                id='city-dropdown', 
                options=[{'label': 'All', 'value': 'All'}],
          ...
          dcc.Dropdown(
                ...
                multi = True,optionHeight=40,
                style={'font-size':'25px','cursor':'pointer','width':'1000px','height':'55px','backgroundColor':'transparent','borderRadius':'10px','padding':'6px'}
                  )],style={'border':'5px solid black','border-radius':'5px','padding':'10px','background-image':' url("/assets/ter7.jpg")'}),

          html.H5('Select the Year', id='year_title', style={'font-family':'cambria','font-size':'20px'}),
          dcc.RangeSlider(
                    id='year-slider',
                    min=min(year_list),
                    max=max(year_list),
      ...
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart", children=[
          dcc.Tabs(id = "subtabs2", value = "WorldChart",children = [
            ...
            dcc.Dropdown(id="Chart_Dropdown", options = chart_dropdown_values, placeholder="Select option", value = "region_txt"
                         ,style={'width':'400px' ,'borderRadius':'15px','bottom':'1px','height':'55px','margin-top':'30px'}), 
            html.Br(),
            html.Hr(),
            dcc.Input(id="search", placeholder="Search Filter",
                      style={'width':'400px','borderRadius':'14px','height':'45px','font-size':'22px'}),
            html.Hr(),
            html.Br(),
            dcc.RangeSlider(
                    id='cyear_slider',
                  ...
                  html.Br()
              ], style={'font-family':'cambria','font-size':'25px'}),
         ],style={'font-size':'25px','border':'2px solid indigo','border-radius':'4px','font-family':'cambria'}),
  html.Div(id = "graph-object", children ="Graph will be shown here" ,style={'font-family':'cambria','font-size':'20px'})
  ], style={'font-family':'cambria','font-size':'25px'})
...
def main8():
  load_data()  # before opening my browser load my data
  ...
  #changing fevicon
    # goto website https://www.favicon.cc/  ---------> create your favicon or goto import image select brows upload your picture
    #then download / save
    #create a directory by the name of assets & copy your favicon.ico(ico means icon)
